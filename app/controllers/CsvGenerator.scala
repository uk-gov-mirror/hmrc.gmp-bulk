...
trait CsvGenerator {
  ...
  class RowBuilder {
    ...
    private val cells = new ListBuffer[Cell]()
    private var errorCell: Option[Cell] = None
    private var errorResolutionCell: Option[Cell] = None
    ...
    def setErrorCell(cell: Cell) = this.errorCell = Some(cell)
    def setErrorResolutionCell(cell: Cell) = errorResolutionCell = Some(cell)
...
//    })).mkString(",")
  ...
  class CsvBuilder(cellCount: Int) {
    ...
    def build: String = {
      rows map { row =>
        val r = row.toString + ("," * (cellCount - row.cells.size))
        println()
        println(s"Cell count: ${row.cells.size}")
        println(Console.GREEN + r + Console.WHITE)
        r
      } mkString "\n"
    }
  }
